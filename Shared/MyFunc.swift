...
class MyFunc {
  ...
  static func getPlayingAreas() -> [PlayingArea] {
  ...
  static func deletePlayingArea(playingAreaId: UUID) {
    ...
    defaults.removeObject(forKey: keyStr)
    MyFunc.logMessage(.debug, "Playing area \(playingAreaIdStr) deleted")
