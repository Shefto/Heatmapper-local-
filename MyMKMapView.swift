...
public class MyMKMapView : MKMapView, MKMapViewDelegate {
  ...
  public func getZoom() -> Double {
    ...
    let angleRad = .pi * angleCamera / 180 // map rotation in radians
  ...
  public func getRotation() -> Double? {
    ...
    if self.mapContainerView != nil {
      var rotation = fabs(180 * asin(Double(self.mapContainerView!.transform.b)) / .pi)
  ...
  private func startTrackingChanges() {
    ...
    if self.changesTimer == nil {
      self.changesTimer = Timer(timeInterval: 0.1, target: self, selector: #selector(MyMKMapView.trackChanges), userInfo: nil, repeats: true)
